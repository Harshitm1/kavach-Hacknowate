...
function init() {
  ...
  getCurrentTab().then(tab => {
    ...
    // Verify that ChatbotService is available
    if (!window.ChatbotService) {
      console.error("ChatbotService not found! Will try to load it again");
      // Try to load the service dynamically if needed
      loadChatbotService();
    } else {
      console.log("ChatbotService is available");
      // Test the API connection
      window.ChatbotService.testConnection()
        .then(isConnected => {
          if (isConnected) {
            console.log("Successfully connected to Agent.ai API");
            // Add a small indicator that the service is connected
            if (elements.chatbotToggle) {
              elements.chatbotToggle.style.backgroundColor = "#4CAF50";
            }
          } else {
            console.error("Failed to connect to Agent.ai API");
            if (elements.chatbotToggle) {
              elements.chatbotToggle.style.backgroundColor = "#F44336";
            }
          }
        })
        .catch(error => {
...
function setupChatbot() {
  try {
    ...
    // Set up send button and input field
    if (elements.chatbotSend && elements.chatbotInput) {
      console.log("Setting up chatbot input handlers");
    ...
    if (elements.chatbotMessages) {
      console.log("Initializing chatbot messages");
...
// Dynamically load the chatbot service if needed
function loadChatbotService() {
  try {
    const script = document.createElement('script');
    script.src = '../js/chatbot-service.js';
    script.onload = function() {
      console.log("ChatbotService loaded dynamically");
      if (window.ChatbotService) {
        console.log("ChatbotService is now available");
      } else {
        console.error("ChatbotService still not available after loading");
      }
    };
    script.onerror = function() {
      console.error("Failed to load ChatbotService script");
    };
    document.head.appendChild(script);
  } catch (error) {
    console.error("Error loading chatbot service:", error);
  }
}
...
function sendChatbotMessage() {
  try {
    if (!elements.chatbotInput || !elements.chatbotInput.value.trim()) return;
    ...
    // Check if ChatbotService is available
    if (!window.ChatbotService) {
      console.error("ChatbotService not available when sending message");
      removeMessage(loadingId);
      addChatbotMessage("I'm having trouble connecting to my services. Please try refreshing the page.", 'bot');
    ...
    try {
      ...
      console.error("Error calling ChatbotService:", error);
      removeMessage(loadingId);
      addChatbotMessage("There was a problem processing your request. Please try again.", 'bot');
    }
  } catch (error) {
    console.error("Error in sendChatbotMessage:", error);
    addChatbotMessage("An unexpected error occurred. Please try again later.", 'bot');
  }
}
...
function getSecurityAdvice() {
  try {
    ...
    // Check if ChatbotService is available
    if (!window.ChatbotService) {
      console.error("ChatbotService not available when getting security advice");
      removeMessage(loadingId);
      addChatbotMessage("I'm having trouble analyzing this site. Please try refreshing the page.", 'bot');
      // Try to load it again
      loadChatbotService();
      return;
    }
    ...
    console.error("Error in getSecurityAdvice:", error);
    addChatbotMessage("An unexpected error occurred while analyzing this site. Please try again later.", 'bot');
  }
}
