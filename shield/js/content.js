...
function showSecurityNotification() {
  ...
  ${issues.map(issue => `- ${issue}`).join('\n')}`;
          ...
          .then(data => {
            let advice = "Stay safe while browsing with Shield's protection.";
            ...
            // Create notification
            showNotification("Shield Security Alert", advice);
          })
          .catch(error => {
            console.error("Error getting security advice:", error);
            showNotification("Shield Security Alert", "Stay vigilant while browsing this site. Enable all Shield protections for maximum security.");
          });
        }
      }
    });
